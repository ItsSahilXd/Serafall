...
import SerafallRobot.modules.sql.notes_sql as sql
from SerafallRobot import dispatcher, LOGGER, OWNER_ID, JOIN_LOGGER, SUPPORT_CHAT
from SerafallRobot.__main__ import DATA_IMPORT
from SerafallRobot.modules.helper_funcs.chat_status import user_admin
from SerafallRobot.modules.helper_funcs.alternate import typing_action

# from SerafallRobot.modules.rules import get_rules
import SerafallRobot.modules.sql.rules_sql as rulessql

# from SerafallRobot.modules.sql import warns_sql as warnssql
import SerafallRobot.modules.sql.blacklist_sql as blacklistsql
from SerafallRobot.modules.sql import disable_sql as disabledsql

# from SerafallRobot.modules.sql import cust_filters_sql as filtersql
# import SerafallRobot.modules.sql.welcome_sql as welcsql
import SerafallRobot.modules.sql.locks_sql as locksql
...
def export_data(update: Update, context: CallbackContext):
    ...
    with open("SerafallRobot{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("SerafallRobot{}.backup".format(chat_id), "rb"),
        caption="*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `SerafallRobot-Backup` was specially made for notes.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("SerafallRobot{}.backup".format(chat_id))  # Cleaning file
